#!/usr/bin/env python
####
# borrowed and stripped from cloud client: https://pypi.python.org/pypi/cloud/
# This file adds a variety of features over regular urllib/httplib,
#  but purpose in dropblox is solely persistent HTTP connections
...
import logging
cloudLog = logging.getLogger('dropblox.HTTPConnection')


CHUNK_SIZE = 65536

http_close_connection = False # seetting to true is default urllib behabior 
...
def makeBodyFunction(v_vars, v_files, boundary):
    def theBody(httpcon):
        try:
            send_data(v_vars, v_files, boundary, sock = httpcon.sock)
            ...
            if type(data) == str:
                ...
                if len(v_files) == 0:
                    ...
                    if not request.has_header('Content-type'):
                        ...
                        data, content_len = send_data(v_vars, v_files, boundary)
                        request.add_unredirected_header('Content-Type',
